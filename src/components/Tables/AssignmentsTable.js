...
class AssignmentsTable extends Component {
    ...
    toggleTableRowHover = (domElement, color) => {
        Object.assign(domElement.style, { cursor: 'pointer', background: color })
    }
                ...
                <Table hover responsive>
                    ...
                    {this.state.assignments.length > 0 ?
                        this.state.assignments.map((assignment, i) => {
                            if (!assignment.published && isStudent(this.state.rolePerms)) return null;
                            ...
                            return (
                                <tr onClick={(event) => this.assignmentDetailRedirect(event, assignment.id)}
                                    onMouseEnter={(e) => this.toggleTableRowHover(e.currentTarget, "#f5f7fa")}
                                    onMouseLeave={(e) => this.toggleTableRowHover(e.currentTarget, "#fff")}
                                >
