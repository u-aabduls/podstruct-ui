...
class addQuestionForm extends Component {
    ...
    setMCAnswer = (event) => {
        ...
        if (!this.state.formAddQuestion['choice' + event.target.value]) return;
    ...
    setMAAnswer = (event) => {
        ...
        for (let i = 0; i < this.state.numberOfChoices; i++) {
            if (!this.state['answer' + (i + 1)]) {
                stateCopy['answer' + (i + 1)] = event.target.value;
                this.setState(stateCopy);
                break;
            }
        }
    ...
    validateMCQuestion = () => {
        ...
        for (let i = 0; i < this.state.numberOfChoices; i++) {
            console.log('choice' + this.alphabet[i])
            console.log(this.state.formAddQuestion['choice' + this.alphabet[i]])
    ...
    validateMAQuestion = () => {
        ...
        var isNullAnswer = false;
        var isNullChoice = true;
        var tooManyAnswers = false;
        if (!this.state.formAddQuestion.answer1) isNullAnswer = true;
        for (let i = 0; i < this.state.numberOfChoices; i++) {
            if (this.state.formAddQuestion['choice' + this.alphabet[i]]) numberOfChoices += 1;
            if (this.state.formAddQuestion['answer' + (i + 1)]) numberOfAnswers += 1;
            if (numberOfChoices >= 2) {
                isNullChoice = false;
            }
        ...
        stateCopy.choices.error.isNullAnswer = isNullAnswer ? true : false;
        stateCopy.choices.error.isNullChoice = isNullChoice ? true : false;
        stateCopy.choices.error.tooManyAnswers = tooManyAnswers ? true : false;
        this.setState(stateCopy);
        return isNullAnswer || isNullChoice || tooManyAnswers;
    }
                                    ...
                                    return (
                                        <div className="form-group">
                                            ...
                                            <div className="input-group with-focus">
                                                ...
                                                {this.state.formAddQuestion.choices.error.isNullAnswer && <p style={this.errorMessageStyling}>Answer is required</p>}
                                                {this.state.formAddQuestion.choices.error.isNullChoice && <p style={this.errorMessageStyling}>Two choices are required</p>}
                                                {this.state.formAddQuestion.choices.error.tooManyAnswers && <p style={this.errorMessageStyling}>Minimum 1 incorrect choice is required</p>}
                                                <div className="input-group">
                                                    <input className="mr-2" type="checkbox" value={e} name="answer" onChange={this.setMAAnswer} />
                                                    <label className="text-muted pt-2">Correct Answer</label>
                                                </div>
                                            </div>
                                        </div>)
